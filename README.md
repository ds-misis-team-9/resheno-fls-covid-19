...
В **Yandex Wordstat** сбор исторических данных через API невозможен. Однако возможно собрать данные пользователю, имеющему учётную запись на yandex. Частично автоматизировать этот процесс помогает [скрипт](https://github.com/ds-misis-team-9/yandex-wordstat-data-collector/blob/master/main.py).
...
Используя методы библиотеки TensorFlow и представленное на сайте[ www.tensorflow.org](http://www.tensorflow.org/) [руководство по анализу временных рядов](https://www.tensorflow.org/tutorials/structured_data/time_series), мы построили различные модели сверточных и реккурентных нейронных сетей и сравнили полученные результаты.
